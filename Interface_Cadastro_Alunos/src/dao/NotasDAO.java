...
public class NotasDAO {
    ...
    public void adicionar(Notas notas) {
        String sql = "INSERT INTO b'cadastro_alunos' (id_aluno, id_curso, nota, data_lancamento) VALUES (?, ?, ?, ?, ?)";
        try {
            PreparedStatement stmt = conexao.prepareStatement(sql);
            stmt.setString(1, notas.getId_aluno());
            stmt.setString(2, notas.getId_curso());
            stmt.setString(3, notas.getNota());
    ...
    public void alterar(Notas notas) {
        String sql = "UPDATE b'cadastro_alunos' SET id_aluno = ?, id_curso = ?, nota = ?, data_lancamento = ? WHERE id = ?";
        try {
            PreparedStatement stmt = conexao.prepareStatement(sql);
            stmt.setString(1, notas.getId_aluno());
            stmt.setString(2, notas.getId_curso());
